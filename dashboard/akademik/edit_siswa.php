...
if (!$student_data) {
    $_SESSION['error_message'] = 'Siswa tidak ditemukan atau tidak memiliki akses!';
    header('Location: siswa.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $checkStmt = $conn->prepare("SELECT id FROM users WHERE username = ? AND id != ? AND sekolah_id = ?");
    $checkStmt->bind_param("sii", $username, $id, $sekolah_id);
    $checkStmt->execute();
    if ($checkStmt->get_result()->num_rows > 0) {
        $checkStmt->close();
        $message = 'error:Username sudah digunakan oleh siswa lain!';
    } else {
        $checkStmt->close();
                ...
                $(document).ready(function() {
                    $('#editSiswaForm').on('submit', function(e) {
                        ...
                        Swal.fire({
                            ...
                            text: 'Apakah Anda yakin ingin menyimpan perubahan data siswa ini?',
                            icon: 'question',
