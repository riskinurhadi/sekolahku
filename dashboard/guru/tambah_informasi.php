...
$(document).ready(function() {
    ...
    $('#tambahInformasiForm').on('submit', function(e) {
        ...
        const target_role = $('#target_role').val();
        const target_user_id = $('#target_user_id').val();
        ...
        // Validasi jika target_role adalah spesifik
        if (target_role === 'spesifik' && !target_user_id) {
            e.preventDefault();
            Swal.fire({
                icon: 'error',
                title: 'Error!',
                text: 'Silakan pilih user target jika memilih "User Spesifik"!'
            });
            return false;
        }
    });
